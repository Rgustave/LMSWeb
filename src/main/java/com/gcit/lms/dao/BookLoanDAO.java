...
public class BookLoanDAO extends BaseDAO implements ResultSetExtractor<List<BookLoan>> {
	...
	public void returnCheckedOutBook(Book bk, Borrower br, LibraryBranch lb)
		...
		if (bk != null && br != null && lb != null) {
			...
			template.update(
					"UPDATE   tbl_book_loans  SET  dateIn = ? WHERE  bookId = ? "
	...
	public List<BookLoan> readAllCheckOutBooksByCardNo(int CardNo) throws ClassNotFoundException, SQLException {
		return template.query("select * from tbl_book_loans where CardNo = ? AND dateIn IS NULL",
	...
	public List<BookLoan> extractData(ResultSet rs) throws SQLException {
		...
		while (rs.next()) {
			...
			bkl.setBook(rs.getInt("bookId"));
			bkl.setLibraryBranch(rs.getInt("branchId"));
			bkl.setBorrower(rs.getInt("cardNo"));
